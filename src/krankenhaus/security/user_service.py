...
from krankenhaus.config import keycloak_admin_config
from krankenhaus.security.role import Role
from krankenhaus.security.user import User
...
class UserService:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            roles_krankenhaus = [
                role for role in roles if role["name"] == "krankenhaus"
            ]
            # Rolle 'krankenhaus' in Keycloak fuer KeycloakAdmin.assign_client_role()
            # und ist ein dict mit Schluesseln "id", "name", "description", ...
            self.rolle_krankenhaus = roles_krankenhaus[0]
            logger.debug("rolle_krankenhaus={}", self.rolle_krankenhaus)
        except KeycloakConnectionError:
            logger.error("Keine Verbindung zu Keycloak! Ist Keycloak gestartet?")
            self.client_uuid = "N/A"
            self.rolle_krankenhaus = None
    ...
    def create_user(self, user: User) -> str:
        ...
        self.keycloak_admin.assign_client_role(
            user_id=user_id, client_id=self.client_uuid, roles=[self.rolle_krankenhaus]
        )
        return user_id
