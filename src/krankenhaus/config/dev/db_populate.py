...
from krankenhaus.config.config import resources_path
...
from krankenhaus.config.dev_modus import dev_db_populate
...
class DbPopulateService:
    ...
    def _load_csv_files(self) -> None:
        logger.debug("begin")
        tabellen: Final = ["krankenhaus", "adresse", "fachbereich"]
        csv_path: Final = "/init/krankenhaus/csv"
        # siehe extras/compose/postgres/compose.init.yml
        with self.engine_admin.connect() as connection:
            connection.execute(text("SET search_path TO krankenhaus;"))
